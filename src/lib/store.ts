...
import { preprocessItem, buildInvertedIndex, indexItem } from "./preprocessor";
import type { MediaItem, InvertedIndex, RawMediaItem } from "./types";
...
const _map = new Map<string, MediaItem>(
  Array.from(_items, (item) => [item.bildnummer, item] as [string, MediaItem])
);
